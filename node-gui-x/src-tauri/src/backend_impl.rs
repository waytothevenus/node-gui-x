...
enum GuiHotColdController {
    ...
    Cold(WalletRpc<ColdWalletClient>, CommandHandler<WalletRpcHandlesClient<ColdWalletClient>>),
}
...
impl Backend {
    ...
    async fn get_account_info<T: NodeInterface + Clone + Send + Sync + Debug + 'static>(
        ...
        let name = controller
            ...
            .account_names.into_iter()
            .nth(account_index.into_u32() as usize)
            .flatten();

        let addresses = controller
            .get_issued_addresses(account_index).await
            .map_err(|e| BackendError::WalletError(e.to_string()))?
            .into_iter()
            .map(|info| {
                Ok((
        ...
        let transaction_list = controller
            .get_transaction_list(account_index, 0, TRANSACTION_LIST_PAGE_COUNT).await
            .map_err(|e| BackendError::WalletError(e.to_string()))?;
    ...
    async fn add_create_wallet(
        ...
        let (wallet_data, accounts_info, best_block) = match (wallet_type, &self.controller) {
            (WalletType::Hot, ColdHotNodeController::Hot(controller)) => {
                ...
                ).await.map_err(|e| BackendError::WalletError(e.to_string()))?;

                let (wallet_rpc, command_handler, best_block, accounts_info, accounts_data) =
                    self.create_wallet(
                        handles_client,
                        file_path.clone(),
                        &mnemonic,
    ...
    > {
        let wallet_service = WalletService::start(
            ...
            vec![],
            handles_client
        ).await.map_err(|err| BackendError::WalletError(err.to_string()))?;
        ...
        let accounts_data = (0..account_indexes)
            .map(|account_index| {
                let account_index = U31::from_u32(account_index as u32).expect("valid num");
                (AccountId::new(account_index), Self::get_account_data(account_index))
            })
            .collect();
        Ok((wallet_rpc, command_handler, best_block, accounts_info, accounts_data))
    }
    ...
    async fn add_open_wallet(
        ...
        let (wallet_data, accounts_info, best_block, encryption) = match
            (wallet_type, &self.controller)
        {
            (WalletType::Hot, ColdHotNodeController::Hot(controller)) => {
                let handles_client = WalletHandlesClient::new(
                    controller.chainstate.clone(),
    ...
    > {
        ...
        let wallet_rpc = WalletRpc::new(wallet_handle, node_rpc.clone(), chain_config.clone());
        wallet_rpc
            .open_wallet(file_path, None, false).await
            .map_err(|err| BackendError::WalletError(err.to_string()))?;
        tokio::spawn(
            forward_events(
                wallet_events,
        ...
        let encryption_state = match wallet_rpc.remove_private_key_encryption().await {
            ...
            Err(
                wallet_rpc_lib::RpcError::Controller(
    ...
    async fn new_address(
        ...
        let (index, address) = match &mut wallet.controller {
            GuiHotColdController::Cold(w, _) =>
                w
                    .issue_address(account_id.account_index()).await
                    .map_err(|e| BackendError::WalletError(e.to_string()))
    ...
    async fn toggle_staking(
        ...
        if enabled {
            self
                ...
                .start_staking(account_id.account_index()).await
                .map_err(|e| BackendError::WalletError(e.to_string()))?;
        } else {
            self
                .hot_wallet(wallet_id)?
                .stop_staking(account_id.account_index()).await
                .map_err(|e| BackendError::WalletError(e.to_string()))?;
        }
    ...
    async fn send_amount(
        ...
        let SendRequest { wallet_id, account_id, address, amount } = send_request;

        let address = parse_address(&self.chain_config, &address).map_err(|err|
            BackendError::AddressError(err.to_string())
        )?;
        let amount = parse_coin_amount(&self.chain_config, &amount).ok_or(
            BackendError::InvalidAmount(amount)
        )?;
    ...
    async fn stake_amount(
        ...
        let amount = parse_coin_amount(&self.chain_config, &amount).ok_or(
            BackendError::InvalidPledgeAmount(amount)
        )?;

        let cost_per_block = parse_coin_amount(&self.chain_config, &cost_per_block).ok_or(
            BackendError::InvalidCostPerBlockAmount(cost_per_block)
        )?;

        let mpt = PerThousand::from_decimal_str(&mpt).map_err(|_|
            BackendError::InvalidMarginPerThousand(mpt)
        )?;
        ...
        let decommission_address = RpcAddress::new(&self.chain_config, decommission_key).expect(
            "addressable"
        );
        let tx = self
            .hot_wallet(wallet_id)?
    ...
    async fn decommission_pool(
        ...
        let DecommissionPoolRequest { wallet_id, account_id, pool_id, output_address } = request;
    ...
    async fn create_delegation(
        ...
        let CreateDelegationRequest { wallet_id, account_id, pool_id, delegation_address } =
            request;
    ...
    async fn delegate_staking(
        ...
        let DelegateStakingRequest { wallet_id, account_id, delegation_id, delegation_amount } =
            request;

        let delegation_amount = parse_coin_amount(&self.chain_config, &delegation_amount).ok_or(
            BackendError::InvalidAmount(delegation_amount)
        )?;
    ...
    async fn send_delegation_to_address(
        ...
        let SendDelegateToAddressRequest { wallet_id, account_id, address, amount, delegation_id } =
            request;
        ...
        let delegation_id = Address::from_string(&self.chain_config, &delegation_id).map_err(|e|
            BackendError::AddressError(e.to_string())
        )?;
    ...
    async fn submit_transaction(
        ...
        self
            .hot_wallet(wallet_id)?
            .submit_raw_transaction(HexEncoded::new(tx), false, Default::default()).await
            .map_err(|e| BackendError::WalletError(e.to_string()))?;
    ...
    async fn process_request(&mut self, request: BackendRequest) {
        match request {
            BackendRequest::OpenWallet { file_path, wallet_type } => {
            ...
            BackendRequest::RecoverWallet { mnemonic, file_path, import, wallet_type } => {
                let import_res = self.add_create_wallet(
                    file_path,
                    mnemonic,
                    wallet_type,
                    import
            ...
            BackendRequest::SendDelegationToAddress(request) => {
                ...
                Self::send_event(&self.event_tx, BackendEvent::SendDelegationToAddress(result));
            }
            ...
            BackendRequest::TransactionList { wallet_id, account_id, skip } => {
                let transaction_list_res = self.load_transaction_list(
                    wallet_id,
                    account_id,
                    skip
                ).await;
            ...
            BackendRequest::ConsoleCommand { wallet_id, account_id, command } => {
    ...
    async fn update_wallets(&mut self) {
        for (wallet_id, wallet_data) in self.wallets
            .iter_mut()
            .filter(|(_, wallet_data)| wallet_data.updated) {
            let controller = match &mut wallet_data.controller {
                GuiHotColdController::Cold(_, _) => {
                    continue;
                }
        ...
        for (wallet_id, wallet_data) in self.wallets.iter_mut() {
            ...
            for (account_id, account_data) in wallet_data.accounts
                .iter_mut()
                .filter(|(_account_id, account_data)| {
                ...
                let delegations_res = controller.list_delegation_ids(
                    account_id.account_index()
                ).await;
...
async fn get_account_balance<N: NodeInterface + Clone + Send + Sync + 'static>(
    ...
    controller
        .get_balance(account_index, UtxoState::Confirmed.into(), WithLocked::Unlocked).await
        .map_err(|e| BackendError::WalletError(e.to_string()))
}
...
async fn encrypt_action<T: NodeInterface + Clone + Send + Sync + 'static>(
    ...
    (
        match action {
            EncryptionAction::SetPassword(password) =>
                controller
                    .encrypt_private_keys(password).await
                    .map(|()| (wallet_id, EncryptionState::EnabledUnlocked)),
            EncryptionAction::RemovePassword =>
                controller
                    .remove_private_key_encryption().await
                    .map(|()| (wallet_id, EncryptionState::Disabled)),
            EncryptionAction::Unlock(password) =>
                controller
                    .unlock_private_keys(password).await
                    .map(|()| (wallet_id, EncryptionState::EnabledUnlocked)),
            EncryptionAction::Lock =>
                controller
                    .lock_private_keys().await
...
async fn select_acc_and_execute_cmd<N>(
    ...
    where N: NodeInterface + Clone + Send + Sync + Debug + 'static
{
    c
        .handle_manageable_wallet_command(
            chain_config,
        ...
        .map_err(|e| BackendError::WalletError(e.to_string()))?;
    c.handle_manageable_wallet_command(chain_config, command).await.map_err(|e|
        BackendError::WalletError(e.to_string())
    )
}
